...
const navItems = [
  ...
  { label: "Discovery Grid", href: "/discovery-grid" },
...
export default function SiteNavBar({ activeItem }: SiteNavBarProps) {
  ...
  const linkClass = (active: boolean) =>
    `group relative rounded-full px-4 py-2 text-sm font-medium tracking-normal transition-all duration-300 ${
      active ? "nav-link-active" : "nav-link-default"
    }`;

  return (
    <>
      <header className="fixed left-0 right-0 top-0 z-50 px-4 pt-2 md:px-8 md:pt-2">
        <div
          className={`navbar-shell mx-auto flex max-w-[1400px] items-center justify-between gap-6 rounded-full px-3 py-2 transition-all duration-500 md:px-5 md:py-2.5 ${
        ...
        >
          ...
          >
            <span
              className={`absolute h-0.5 w-4 bg-white transition-all duration-300 ${open ? "rotate-45" : "-translate-y-1"}`}
            />
            <span
              className={`absolute h-0.5 w-4 bg-white transition-all duration-300 ${open ? "opacity-0" : ""}`}
            />
            <span
              className={`absolute h-0.5 w-4 bg-white transition-all duration-300 ${open ? "-rotate-45" : "translate-y-1"}`}
            />
                ...
                return (
                  <li key={item.label}>
                    <Link
                      href={item.href}
                      onClick={() => setOpen(false)}
                      className={`group block rounded-2xl px-4 py-3 text-sm font-medium tracking-normal transition-all duration-300 ${
                        active ? "nav-link-active" : "nav-link-default"
                      }`}
                    >
                      <span className={active ? "devora-gradient-text" : ""}>
